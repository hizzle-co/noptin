...
class Pagination extends Component {
	...
	renderPageArrows() {
		...
		const previousLinkClass = classNames( 'noptin-pagination__link', {
			'is-active': page > 1,
		} );

		const nextLinkClass = classNames( 'noptin-pagination__link', {
			'is-active': page < this.pageCount,
		} );

		return (
			<div className="noptin-pagination__page-arrows">
				{ showPageArrowsLabel && (
					<span
						className="noptin-pagination__page-arrows-label"
						role="status"
						aria-live="polite"
					>
						{ sprintf(
				...
				<div className="noptin-pagination__page-arrows-buttons">
					<Button
	...
	renderPagePicker() {
		...
		const inputClass = classNames(
			'noptin-pagination__page-picker-input',
			{
				'has-error': isError,
			}
		);

		const instanceId = uniqueId( 'noptin-pagination-page-picker-' );
		return (
			<div className="noptin-pagination__page-picker">
				<label
					htmlFor={ instanceId }
					className="noptin-pagination__page-picker-label"
				>
					{ __( 'Go to page', 'newsletter-optin-box' ) }
					<input
						id={ instanceId }
	...
	renderPerPagePicker() {
		...
		return (
			<div className="noptin-pagination__per-page-picker">
				<SelectControl
