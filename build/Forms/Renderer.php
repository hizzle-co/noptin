...
class Renderer {
	 ...
	 * @param Form|false $form The form to display.
	 *
	 * @return string
	 */
	...
	protected static function get_css_classes( $args, $form ) {
		...
		if ( $form ) {
			return array(
				...
				$form->unstyled ? 'noptin-form-unstyled' : '',
				$args['html_id'],
			);
		}
	...
	/**
	 * Formats text for the form.
	 *
	 * @param string $text The text to format.
	 */
	protected static function format_text( $text ) {
		return do_shortcode(
			apply_filters(
				'noptin_form_text',
				$text
					...
					<div class="noptin-form-header <?php echo ! empty( $form->image ) ? esc_attr( "noptin-img-{$form->imagePos}" ) : 'no-image'; ?>">
						...
						<div class="noptin-form-header-text">
							...
							<?php if ( ! $form->hidePrefix ) : ?>
								<div style="<?php echo esc_attr( self::get_typography_css( $form->prefixColor, $form->prefixTypography ) ); ?>" class="noptin-form-prefix">
									<?php echo wp_kses_post( self::format_text( $form->prefix ) ); ?>
								</div>
							<?php endif; ?>

							<?php if ( ! $form->hideTitle ) : ?>
								<div style="<?php echo esc_attr( self::get_typography_css( $form->titleColor, $form->titleTypography ) ); ?>" class="noptin-form-heading">
									<?php echo wp_kses_post( self::format_text( $form->title ) ); ?>
								</div>
							<?php endif; ?>

							<?php if ( ! $form->hideDescription ) : ?>
								<div style="<?php echo esc_attr( self::get_typography_css( $form->descriptionColor, $form->descriptionTypography ) ); ?>" class="noptin-form-description">
									<?php echo wp_kses_post( self::format_text( $form->description ) ); ?>
								</div>
							<?php endif; ?>
	...
	protected static function display_fields( $args, $form ) {
		...
		$is_single_line = $form && $form->singleLine;
		$current_email  = '';
		...
		if ( ! $form || empty( $form->hideFields ) ) {
			...
			foreach ( $fields as $custom_field ) {
				...
				if ( $form && ! empty( $custom_field['type'] ) ) {
		...
		if ( $form ) {
			...
			<?php if ( ! $form->hideNote && ! empty( $form->note ) ) : ?>
				<div style="<?php echo esc_attr( self::get_typography_css( $form->noteColor, $form->noteTypography ) ); ?>" class="noptin-form-note">
					<?php echo wp_kses_post( self::format_text( $form->note ) ); ?>
				</div>
			<?php endif; ?>
	...
	protected static function display_submit_button( $wrap, $args, $form ) {
		...
		if ( $form ) {
			...
			if ( ! empty( $form->noptinButtonBgGradient ) ) {
				$button_atts['style'] .= 'background: ' . $form->noptinButtonBgGradient . ';';
			} elseif ( ! empty( $form->noptinButtonBg ) ) {
			...
			if ( ! empty( $form->noptinButtonRadius ) ) {
				$button_atts['style'] .= 'border-radius: ' . $form->noptinButtonRadius . ';';
			}

			if ( ! empty( $form->noptinButtonShadow ) ) {
				$button_atts['style'] .= 'box-shadow: ' . $form->noptinButtonShadow . ';';
			}
	...
	private static function before_display( $form, $args = array() ) {
		...
		$has_field_styles = ! empty( $form->fieldBg ) || ! empty( $form->fieldTextColor ) || ! empty( $form->fieldBorder['generated'] );
		if ( ! empty( $form->CSS ) || ! empty( $attr ) || $has_field_styles ) {
			$id_class   = sprintf( 'noptin-form-id-%d', $form->id );
			$type_class = sprintf( 'noptin-%s-main-wrapper', $form->optinType );

			// Build extra scoped CSS for field and button styles.
			$extra_css = '';

			$field_css = '';
			if ( ! empty( $form->fieldBg ) ) {
				$field_css .= 'background-color: ' . $form->fieldBg . ';';
			}
			if ( ! empty( $form->fieldTextColor ) ) {
				$field_css .= 'color: ' . $form->fieldTextColor . ';';
			}
			if ( ! empty( $form->fieldBorder['generated'] ) ) {
				$field_css .= $form->fieldBorder['generated'];
			}
			if ( ! empty( $field_css ) ) {
				$extra_css .= sprintf( '.%s .noptin-optin-form-wrapper .noptin-form-field { %s }', $id_class, $field_css );
			}

			printf(
				'<style>%s%s #%s__wrapper {%s}</style>',
				$extra_css,
		...
		$wrapper_styles = '';
		foreach ( array_filter( $styles ) as $prop => $val ) {
			$val             = esc_attr( $val );
			$wrapper_styles .= " $prop:$val;";
		}

		if ( ! empty( $form->noptinFormBgGradient ) ) {
			$wrapper_styles .= 'background: ' . esc_attr( $form->noptinFormBgGradient ) . ';';
		}

		if ( ! empty( $form->formBoxShadow ) ) {
			$wrapper_styles .= 'box-shadow: ' . esc_attr( $form->formBoxShadow ) . ';';
		}
