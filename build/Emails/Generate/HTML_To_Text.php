...
class HTML_To_Text {
	...
	*/
	...
	protected function buildlinkList( $link, $display, $linkOverride = null ) {
		...
		// Ignored link types
		if ( preg_match( '!^(javascript:|mailto:|#)!i', html_entity_decode( $link, ENT_QUOTES | ENT_HTML5, self::ENCODING ) ) ) {
			return $display;
		}
			...
			foreach ( $matches[0] as $m ) {
				...
				if ( '<' === $m[0][0] && '/' === $m[0][1] ) {
					...
					if ( 0 > $level ) {
						...
						$body = preg_replace( '/((^|\n)>*)/', '\\1> ', trim( $body ) );
						$body = '<pre>' . htmlspecialchars( $body, ENT_QUOTES | ENT_HTML5, self::ENCODING ) . '</pre>';
						// Re-set text width
	...
	protected function strtoupper( $str ) {
		$str = html_entity_decode( $str, ENT_QUOTES | ENT_HTML5, self::ENCODING );
		$str = mb_strtoupper( $str );
		$str = htmlspecialchars( $str, ENT_QUOTES | ENT_HTML5, self::ENCODING );
