		...
		if ( $this->is_mass_mail() && 'newsletter' !== $this->type ) {
			...
			$args   = array_merge(
				...
				array(
					...
					'content'      => 'visual' === $type ? noptin_parse_email_content_tags( Generate\Generator::handle_item_lists_shortcode( $this->content ), true ) : '',
					'author'       => $this->author,
			...
			foreach ( array_keys( get_noptin_email_types() ) as $email_type ) {
				...
				$value = Generate\Generator::handle_item_lists_shortcode( $args[ $key ] );
